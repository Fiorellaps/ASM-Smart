...
export const StepsViewTable: React.FC<Props> = (props) => {
  const { tests } = props;
  ...
  const columns = useMemo(
    () => [
      {
        accessorFn: (row) => `${row.idPaso}`, //accessorFn used to join multiple data into a single cell
        id: 'idPaso', //id is still required when using accessorFn instead of accessorKey
        header: 'Paso',
        size: 100,
